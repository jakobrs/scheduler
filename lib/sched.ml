...
module Runtime : sig
  ...
  module Epoll = struct
    ...
    let task ctx wait_count =
      while !(ctx.live_tasks) > 1 do
        let timeout = if wait_count () = 0 then -1 else 0 in
  ...
  let run (f : unit -> unit) : unit =
    ...
    let ctx = { sched; ep; live_tasks } in
    ...
    let count_waiting_tasks () = Queue.length queue in
    let epoll_task = fiber (fun () -> Epoll.task ctx count_waiting_tasks) in
